...
<?php  
    if(isset($_POST['submit'])){
        ...
        // Phải chỉnh sửa lại
        $mailer->Username = "hajjjjju@gmail.com"; // GMAIL username
        $mailer->Password = "hxgwndgtiksfxlck"; // GMAIL password
        $mailer->AddAddress($email, $ten); //email người nhận, $email và $ten là 2 biến đc gán bởi $_POST lấy từ trong form
        $mailer->AddCC("hajjjjju@gmail.com", "Admin Vua máy tính"); // gửi thêm một email cho Admin

        // Chuẩn bị gửi thư nào
        $mailer->FromName = 'Vua Máy Tính'; // tên người gửi
        $mailer->From = 'hajjjjju@gmail.com'; // mail người gửi
        $mailer->Subject = 'Hóa đơn xác nhận mua hàng từ Vua máy tính';
        ...
        if(!$mailer->Send()){
            ...
            //gửi thành công
            header('location: /index4.php?page_layout=hoanthanh'); 
        }
        }
    }
?>
